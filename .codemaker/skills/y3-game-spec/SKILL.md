...
开发完成后，必须执行以下标准测试循环，直到所有 Trace 错误被修复。
...
#### 步骤 1：检查游戏状态
```
MCP 工具: y3-helper.get_game_status
```

根据返回结果：
- **游戏未运行** → 执行 `y3-helper.launch_game` 启动游戏
- **游戏已运行** → 执行 `y3-helper.quick_restart` 快速重启

#### 步骤 2：等待运行稳定
等待 **20 秒**，让游戏完成初始化并运行一段时间，积累足够的日志。

#### 步骤 3：获取日志
```
MCP 工具: y3-helper.get_logs
参数: { "limit": 100 }
```

获取最近的游戏日志，重点关注 **Trace** 错误。
...
| 关键词 | 含义 | 处理方式 |
|--------|------|----------|
| `[LUA_ERROR]` | Lua 运行时错误 | 定位堆栈，修复代码 |
| `stack traceback` | 错误堆栈 | 分析调用链 |
| `attempt to index nil` | 空值访问 | 检查变量初始化 |
| `attempt to call nil` | 调用不存在的函数 | 检查 API 拼写 |

**修复后，必须重复测试循环（回到步骤 1）**

#### 步骤 5：效果验证
当 **连续一轮测试无 Trace 错误** 后，开始验证实际效果：
- 检查游戏逻辑是否符合预期
- 检查 UI 显示是否正确
- 检查交互是否流畅
...
| **循环直到通过** | 每次修复后必须重新跑完整测试流程 |
| **等待充足** | 20 秒等待不可省略，确保日志完整 |
| **记录问题** | 将 Trace 问题记录到 `lua-issues/trace_issues.md` |

### 快速参考命令

```
# 1. 检查状态
y3-helper.get_game_status

# 2a. 启动游戏（如果未运行）
y3-helper.launch_game

# 2b. 快速重启（如果已运行）
y3-helper.quick_restart

# 3. 等待 20 秒后获取日志
y3-helper.get_logs { "limit": 100 }

# 4. 检查 Lua 语法（可选，在修复代码后）
y3-helper.read_problems_lua { "pathGlob": "maps/EntryMap/script" }
```
