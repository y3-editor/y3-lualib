...
class UITreeGenerator:
    ...
    @staticmethod
    def extract_tree(node: Dict[str, Any]) -> Dict[str, Any]:
        """
        递归提取节点的关键信息
        ...
        if "children" in node and node["children"]:
            result["children"] = [
                UITreeGenerator.extract_tree(child) 
                for child in node["children"]
            ]
        
        return result
    
    def process_file(self, ui_file: Path) -> Optional[Dict[str, Any]]:
        """
        处理单个 UI 文件
        ...
        try:
            ...
            # 提取树结构
            tree = self.extract_tree(ui_data)
            return tree
    ...
    def generate(self):
        ...
        for i, ui_file in enumerate(ui_files, 1):
            ...
            # 提取树结构
            tree = self.process_file(ui_file)
            if tree is None:
                print("❌")
                continue
