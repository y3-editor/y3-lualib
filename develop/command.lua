...
New 'ECAFunction' 'gmcommand'
    -- 声明第一个参数的ECA类型
    : with_param('单位', 'string')
    : with_param('单位', 'Player')

    : call(function (string,player)
       
        if not y3.util.stringStartWith(string, '.') then
            return
        end
    
        local content = string:sub(2)
        local strs = {}
        for str in content:gmatch('[^%s]+') do
            strs[#strs+1] = str
        end
...
y3.game:event({y3.const.GlobalEventType.CUSTOM_EVENT,1301971163}, function(_, data)
    print('2222222')
    local des = y3.get_custom_param(data, "内容")
    if not y3.game.is_debug_mode() then
        return
    end
    if not y3.util.stringStartWith(des, '.') then
        return
    end
    local content = des:sub(2)
    local strs = {}
    for str in content:gmatch('[^%s]+') do
        strs[#strs+1] = str
    end
