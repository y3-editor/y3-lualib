...
function 表.获取最小可用索引(表, 最大值)
    最大值 = 最大值 or 100
    for i = 1, 最大值, 1 do
        if 表.是否存在字段(表, i) == false then
            return i
        end
    end
    return -1
...
---@return integer 表长度
function 表.排序遍历(待排序表, 数值回调, 遍历回调, 降序)
    local 临时表, 插入值
    if 降序 then
        临时表 = { { 对比值 = -9999999999 } }
        插入值 = function(key, 对比值)
            for index, value in ipairs(临时表) do
                if 对比值 > value.对比值 then
                    表.插入(临时表, index, { 原字段 = key, 对比值 = 对比值 })
                    break
                end
            end
        end
