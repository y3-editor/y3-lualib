...
---@return Shape
function M.创建环形(in_radius, out_radius)
    local py_shape = GlobalAPI.create_annular_shape(Fix32(in_radius), Fix32(out_radius))
    return M.获取于handle(py_shape)
end

---创建圆形区域
---@param radius number 半径
---@return Shape
function M.创建圆形(radius)
...
---@return Shape
function M.创建矩形(width, length, angle)
...
---@return Shape
function M.创建扇形(radius, angle, direction)
