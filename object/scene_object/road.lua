...
function M.从资源id获取(res_id)
    if not M.map[res_id] then
        ...
        local road = M.从handle获取(py_road)
        road.res_id = res_id
        M.map[res_id] = road
    end
    return M.map[res_id]
...
---@param py_road py.Road
---@return Road
function M.从handle获取(py_road)
    local road = New "Road" (py_road)
    return road
end

y3.py_converter.register_py_to_lua("py.Road", M.从handle获取)
...
---@return Road 创建的路径
function M.创建自起点(start_point)
    -- TODO 见问题2
    ---@diagnostic disable-next-line: param-type-mismatch
    local py_path = GameAPI.create_road_point_list(start_point.handle)
    return M.从handle获取(py_path)
end

---按标签获取所有的路径
---@param tag string 标签
---@return Road[] 路径
function M.获取拥有标签的所有路径(tag)
    local py_list = GameAPI.get_roads_by_tag(tag)
    local roads = y3.helper.unpack_list(py_list, M.从handle获取)
    return roads
end
